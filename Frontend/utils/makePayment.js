...
export const MakePayment = async (PID, fromaddress, toaddress, amount) => {
  ...
  if (amount * 10 ** 18 <= balanceOf) {
    ...
    const MakePayment = await contract.methods
      ...
      //   fromaddress,
      //   toaddress,
      //   BigInt(parseInt(amount) * 0.95 * 10 ** 18)
      // )
      .transfer(toaddress, BigInt(parseInt(amount) * 0.95 * 10 ** 18))
...
export const MaketokenPayment = async (PID, fromaddress, toaddress, amount) => {
  ...
  if (amount * 10 ** 18 <= balanceOf) {
    const MakePayment = await contract.methods
      ...
      .transfer(toaddress, BigInt(parseInt(amount) * 10 ** 18))
      .send({ from: fromaddress });
...
export const PaymentBuyertoSeller = async (
  PID,
  fromaddress,
  toaddress,
  amount
) => {
  console.log(PID, fromaddress, toaddress, amount);
  if (typeof window.ethereum === "undefined") {
    alert("Please install MetaMask first.");
  }
  ...
  if (amount * 10 ** 18 <= balanceOf) {
    const MakePayment = await contract.methods
      ...
      //   fromaddress,
      //   toaddress,
      //   BigInt(parseInt(amount) * 0.05 * 10 ** 18)
      // )
      .transfer(toaddress, BigInt(parseInt(amount) * 0.05 * 10 ** 18))
      .send({ from: fromaddress });
    if (MakePayment != "") {
      console.log(MakePayment);
      console.log(MakePayment["transactionHash"]);
